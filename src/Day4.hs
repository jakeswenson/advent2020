...
validField ("hcl", color) =
  (T.head color == '#')
    && T.all (`elem` "abcdef1234567890") (T.tail color)
validField ("hgt", height)
  | T.takeEnd 2 height == T.pack "in" = parseInt (T.dropEnd 2 height) `elem` [59 .. 76]
  | T.takeEnd 2 height == T.pack "cm" = parseInt (T.dropEnd 2 height) `elem` [150 .. 193]
  | otherwise = False
validField _ = False
